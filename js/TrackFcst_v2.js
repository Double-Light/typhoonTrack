...
/* 建立/更新選單：TD/TY清單 */
// 1. 改為 async 函式
async function gen_TDTY_List() {
  try {
    // 2. 等待資料就緒
    await window.TyphoonData.readyPromise;
    // 3. 從全域物件安全地取得資料
    const { AData } = window.TyphoonData.data;

    if (!AData) {
      console.error("gen_TDTY_List: AData is not available.");
      return;
    }
    ...
    if (Flag_Initial) {
      $("div#data-select").append("<div class='panel-div'>TD/TY：<select id='TyList'></select></div>");
    }
    ...
    console.error("Error in gen_TDTY_List:", error);
  }
};

/* 讀取該年度颱風路徑預報時間(僅列出守視範圍內或有警報估計資料，且有預報資料的時段) */
// 1. 改為 async 函式
async function get_TrackFcst_dict() {
  try {
    // 2. 等待資料就緒
    await window.TyphoonData.readyPromise;
    // 3. 從全域物件安全地取得資料
    const { FData, Warning_Estimate } = window.TyphoonData.data;

    if (!FData || !Warning_Estimate) {
      console.error("get_TrackFcst_dict: FData or Warning_Estimate is not available.");
      return;
    }
    ...
    if (Flag_Initial) {
      $("div#data-select").append("<div class='panel-div'>初始時間：<select id='trackFcstList'></select>(LST)</div>");
    }
    ...
    console.error("Error in get_TrackFcst_dict:", error);
  }
};


/* 建立/更新選單：颱風路徑預報時間清單 */
function gen_TrackFcst_list() {
  let td_num = $("select#TyList option:selected").attr("tdnum");
  let Select_List = "";

  if (typeof td_num === 'string' && trackFcst_dict && trackFcst_dict[td_num]) {
    trackFcst_dict[td_num]['init_times'].sort((a, b) => new Date(b) - new Date(a)).forEach(function(init_time) {
...
async function gen_TrackFcstPage() {
  try {
    ...
    let xInitDate = $("select#trackFcstList option:selected").val() ? $("select#trackFcstList option:selected").val() : null;
    let xInitDate_OBJ = moment(xInitDate);

    $("g#pastPath, g#fcstPath, g#tc_point, g#tc_circle, g#warning_circle, g#warning_marks").contents().remove();
    $("div#keypoint-mark").contents().remove();
    $("#slide-title, #slide-description, #keypoint-content").contents().remove();
    $("#slide-production div").contents().remove();
    $("#keypoint").hide();
    $("div#point-data tbody, #warning_estimate_list").contents().remove();
    ...
    if (xInitDate != null) {
      yPData = AData[yName]["data"].filter((xitem) => moment(xitem["fix_time"]) <= xInitDate_OBJ);
      let yPaths = [];
      yPData.forEach(item => {
      ...
      let xTCData = _.where(FData, { cwb_td_no: td_num });
      if (xTCData && xTCData.length > 0) {
        xPData = _.where(xTCData[0]["data"], { init_time: xInitDate });
        let xPaths = [];
        let xPoints = [];

        xPData.forEach((item, i) => {
          let pointType = item['tau'] == 0 ? 'curr' : 'fcst';
          let ax = roundTo((parseFloat(item["coordinate"][0] - Map_Range[0])) * per_Lon, 2);
          let ay = roundTo((parseFloat(item["coordinate"][1] - Map_Range[3])) * per_Lat, 2);
          let R15 = get_radius(item['circle_of_15ms']);
          let R25 = get_radius(item['circle_of_25ms']);
      ...
      await setSlideTheme(); // Call async version
      await gen_warning(); // Assuming gen_warning might also be async or depend on data
      setEditModel();
      toggleAnimEnable();
    }
  } catch (error) {
    console.error("Error in gen_TrackFcstPage:", error);
  }
};


// 讀取暴風半徑 (新舊版差異)
function get_radius(elem) {
  if (elem === null) {
    return -99;
  } else if (Array.isArray(elem)) {
    return elem[0]['radius'];
  } else if (elem && typeof elem === 'object' && 'radius' in elem) {
    return elem['radius'];
  } else {
    return null;
  }
...
// 依投影片主題設定樣式
// 1. 改為 async 函式
async function setSlideTheme() {
  try {
    // 2. 等待資料就緒
    await window.TyphoonData.readyPromise;
    // 3. 從全域物件安全地取得資料
    const { WData } = window.TyphoonData.data;

    if (!WData) {
      console.error("setSlideTheme: WData is not available.");
      return;
    }
    ...
    let ty_num = $("select#TyList option:selected").attr("tynum") != 'null' ? parseInt($("select#TyList option:selected").attr("tynum")) : null;
    let xInitDate = $("select#trackFcstList option:selected").val() ? $("select#trackFcstList option:selected").val() : null;
    let slide_theme_type = $("select#slide_theme_type option:selected").val();

    if (xInitDate != null) {
      $("#slideObj").attr('slide_theme_type', slide_theme_type);
      $("#slideShps").attr('slide_theme_type', slide_theme_type);
      ...
      if (xPData && xPData.length > 0) {
        ...
        let TcIntensity = "";
        if (xPData[0]['max_wind_speed'] >= 51) TcIntensity = "強烈颱風";
        else if (xPData[0]['max_wind_speed'] >= 32.7) TcIntensity = "中度颱風";
        else if (xPData[0]['max_wind_speed'] >= 17.2) TcIntensity = "輕度颱風";
        else TcIntensity = "熱帶性低氣壓";

        let R15 = get_radius(xPData[0]['circle_of_15ms']).toString().replace("-99", "--");
        let R25 = get_radius(xPData[0]['circle_of_25ms']).toString().replace("-99", "--");

        if (slide_theme_type === "Full_Map_1") {
          $("#slideObj").css("width", 720).css("height", 405);
          $("svg#svgObj").attr("width", 720).attr("height", 405);
          $("#slideShps").show();
          ...
          if (TcIntensity === "熱帶性低氣壓") {
            if (ty_ch_name !== 'null') {
              ...
              if (past_R15_Max > 0) strr = '(原' + ty_ch_name + '颱風)';
              else if (fcst_R15_Max > 0) strr = '(準' + ty_ch_name + '颱風)';
            }
          ...
          $("#slide-production div").html("製作時間：" + (new Date().getFullYear()) + "年" + moment(new Date()).format("MM月DD日HH時") + "，本資料視颱風路徑變化而有調整，實際發生時間以中央氣象署發佈為準。");
      ...
      } else if (slide_theme_type === "Card_1v1") {
        $("#slideObj").css("width", 720).css("height", 720)
        $("svg#svgObj").attr("width", 720).attr("height", 720)
        $("#slideShps").hide()
        $("#slide-title").html('');
        $("#slide-description").html('');
        $("#slide-production div").html('');
      }
    ...
    console.error("Error in setSlideTheme:", error);
  }
}


// 調整SVG ViewBox (警報時間標記建立/變更、投影片、底圖類型變更時啟動)
change_SVG_Size = function() {
  ...
  if ($("g#warning_marks g").length > 0) {  // 有重要時間點 => 內容區域 = marks + 暴風半徑 + 臺灣範圍
    ...
    const cBounds = [xPData[0], ...warning_data]
      .filter(item => typeof item.ax === "number" && typeof item.ay === "number" && typeof item.R15_x === "number" && typeof item.R15_y === "number")  // 位置半徑為數值
      .map(item => [item.ax - item.R15_x, item.ay - item.R15_y, item.ax + item.R15_x, item.ay + item.R15_y])
  ...
  if (slide_theme_type === "Full_Map_1") {
    ...
    whiteSpace = [80, 80, 560, 560] // [x, y, width, height]
  }
  ...
  newViewBox = [(contentSpace[0] + contentSpace[2] / 2) - (whiteSpace[0] + whiteSpace[2] / 2) * newScale,
                ...
                $("#slideObj #svgObj").width() * newScale,
                $("#slideObj #svgObj").height() * newScale
               ]
