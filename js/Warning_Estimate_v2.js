...
setWarningMarksSize = function(fontSize = 9, markName = "") {
  ...
  $(`#warning_marks > g${markName ==="" ? "" : "[name='"+markName+"']"}`).each(function() {
    ...
    // 建立描述文字字串 (用於決定 label寬度與高度)
    let tspans = $(this).find("text tspan").map((_,el)=>$(el).text()).get()
    let textMaxLen = tspans.length ? Math.max(...tspans.map(t=>[...t].reduce((len,ch)=>len+(/[\u0000-\u00ff]/.test(ch)?0.5:1),0))) : 6 ;

    // console.log("最大加權字數：", textMaxLen);
    const lines = $(this).find("text tspan").length;
    ...
    // console.log($(this).attr("name"),ConnX,ConnY,tspans,textMaxLen,ConnectorItem[ConnectorType - 1],labelX,labelY,labelWidth,labelHeight);
    ...
    $(this).find("text tspan").attr("x",labelX + labelWidth / 2)
