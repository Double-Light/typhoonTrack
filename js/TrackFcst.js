...
get_TrackFcst_dict = function() {
  ...
  FData.forEach(typhoon => {
    ...
    if (initTimes.length === 0) {
      $(`#TyList option[tdnum=${cwbTdNo}]`).prop("disabled", true)
    }
  });
  
    // 選取第一個非 disabled
  $("select#TyList option:not([disabled])").first().prop("selected", true)
};
...
setSlideTheme = function() {
  ...
  if (xInitDate != null) {
    ...
    // 變更 #slide slide_theme_type

    $("#slideObj").attr('slide_theme_type', slide_theme_type)
    $("#slideShps").attr('slide_theme_type', slide_theme_type.replace("Card_1v1","Full_Map_1"))
...
set_Basemap_Theme = function() {
  const map_theme_type = $("select#map_theme_type option:selected").val()

  // 底圖為"衛星影像"
  if (map_theme_type === "Satellite_Map"){
    // 調整SVG 地圖填滿與邊界樣式
    $("#slideObj #svgObj").find("g#world, g#nearSea path").css("fill","#FFF0")
    $("#slideObj #svgObj").find("g#world, g#nearSea path, g#臺灣").css("stroke","#FFF")
    ...
    $("#slideObj #svgObj").find("g#g_tc_timestr").css("fill","#FFF")
    ...
    $("#slideObj #svgObj").find("g#world").css("fill","#fff89e")                                   // 還原SVG 地圖填滿樣式
    $("#slideObj #svgObj").find("g#world, g#臺灣").css("stroke","#e0abab")                          // 還原SVG 地圖邊界樣式
    ...
    $("#slideObj #svgObj").find("g#g_tc_timestr").css("fill","#000")
    $("#slideObj #shpObjs").find("#slide-title, #slide-description").css("background-color","rgba(255, 248, 193, .6)")
  }
  
  $("#slideObj").attr("map_theme_type",map_theme_type)
  $("#toggleDisplay>ul").attr("map_theme_type",map_theme_type)
}
